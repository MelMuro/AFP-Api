...
it("should update existing restaurant", async ()=>
	...
	const restaurants = res.body as Restaurant[];
		...
		const updateResponse = await request.put(`/restaurants/${restaurants[0]._id}`).send(updatedRestaurantData);
...
it('should delete restaurant', async () => {
	...
	//delete
    const deleteResponse = await request.delete(`/restaurants/${restaurants[0]._id}`);
  
    expect(deleteResponse.statusCode).toBe(200);
    expect(deleteResponse.text).toContain(`Successfully removed restaurant`);
});
});
